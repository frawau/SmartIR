...
class VerySmartIRClimate(SmartIRClimate):
    def __init__(self, hass, config, hvac, lom=[]):
        ...
        self._hvac_dev = hvac
        self._manufacturer = hvac.brand
        self._supported_models = lom
        self._supported_controller = "Broadlink"
        self._commands_encoding = "Hex"
        self._min_temperature = hvac.capabilities["temperature"][0]
        self._max_temperature = hvac.capabilities["temperature"][-1]
        self._precision = hvac.temperature_step

        # Need translating
        valid_hvac_modes = [
            x if x != "fan" else "fan_only" for x in hvac.capabilities["mode"]
        ]

        self._operation_modes = [x for x in valid_hvac_modes if x in HVAC_MODES]
        if "fan" in hvac.all_capabilities:
            self._fan_modes = hvac.all_capabilities["fan"]
        else:
            self._fan_modes = []
        if "swing" in hvac.all_capabilities:
            self._swing_modes = hvac.all_capabilities["swing"]
        else:
            self._swing_modes = []
        ...
        self._target_temperature = self._hvac_dev.status["temperature"]
        self._hvac_mode = self._hvac_dev.status["mode"]
        if "fan" in self._hvac_dev.status:
            self._current_fan_mode = self._hvac_dev.status["fan"]
        else:
            self._current_fan_mode = None
        if "swing" in self._hvac_dev.status:
            self._current_swing_mode = self._hvac_dev.status["swing"]
            self._support_flags = self._support_flags | ClimateEntityFeature.SWING_MODE
            self._support_swing = True
        else:
            self._current_swng_mode = None
            self._support_swing = False
        self._last_on_operation = None
    ...
    async def send_command(self):
        async with self._temp_lock:
            try:
                self._on_by_remote = False
                self._hvac_dev.set_value("mode", self._hvac_mode)
                self._hvac_dev.set_value("fan", self._current_fan_mode)
                self._hvac_dev.set_value("swing", self._current_swing_mode)
                self._hvac_dev.set_value("temperature", self._target_temperature)

                # Build it
                frames = self._hvac_dev.build_ircode()
                bframe = self._hvac_dev.to_broadlink(frames)
                await self._controller.send(bframe.hex())
                self._hvac_dev.update_status()
                await asyncio.sleep(self._delay)
                return
